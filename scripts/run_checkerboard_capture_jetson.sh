...
# Captured images are pulled back to the laptop's datasets/checkerboard/.
#
...
if [ "${RESET}" = "1" ]; then
  ...
  rm -f "${REPO_DIR}"/datasets/checkerboard/calib_*.jpg 2>/dev/null || true
  ssh "${JETSON_USER}@${JETSON_HOST}" "rm -f ${REMOTE_PKG}/datasets/checkerboard/calib_*.jpg" 2>/dev/null || true
fi
start_stream
...
fetch_from_jetson "datasets/checkerboard/" "${REPO_DIR}/datasets/checkerboard/"
